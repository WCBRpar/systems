  ...
  config = mkIf (countEnabledSites config.mkSite > 0) (lib.mkMerge [
    ...
    # Traefik (galactica)
    {
      services.traefik.dynamicConfigOptions = lib.mkIf (config.networking.hostName == "galactica") {
        http = {
          routers = listToAttrs (mapAttrsToList
            (id: site:
              let
                routerName =
    ...
    # WordPress (pegasus) - definição dos sites via wp4nix
    (lib.mkIf (lib.hasPrefix "pegasus" config.networking.hostName) {
      services.wordpress.webserver = "nginx";
      services.wordpress.sites = listToAttrs (mapAttrsToList
        (id: site:
    ...
    # Definição unificada dos virtual hosts (WordPress + estáticos) para garantir as portas corretas
    (lib.mkIf (lib.hasPrefix "pegasus" config.networking.hostName) {
      services.nginx.virtualHosts = lib.mkMerge [
        (lib.mkForce (listToAttrs (mapAttrsToList
          (id: site:
            nameValuePair site.siteFQDN {
              listen = [{ addr = "0.0.0.0"; port = 7770; ssl = false; }];
              root = "${site.wordpress.package}/share/wordpress";
              serverName = site.siteFQDN;
              locations = {
                "/" = {
                  index = "index.php";
      ...
      # Garantir que os diretórios dos sites estáticos existam
